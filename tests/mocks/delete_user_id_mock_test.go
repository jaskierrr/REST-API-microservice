...
func Test_DeleteUserID(t *testing.T) {
	...
	tests := []struct {
		...
		{
			//? я не понял что я могу тут проверить, если ошибка у меня рождается только если рэббиту плохо, но я так понимаю у меня в хендлере уже стоит определена обработка ошибки на неверный id, поэтому этот тест проходит, так как я насильно ее получаю в EXPECT?
			name: "wrong_ID",
			args: reqArgErr,
			prepare: func(f *fields) {
				gomock.InOrder(
